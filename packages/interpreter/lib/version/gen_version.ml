...
let get_version_from_dune_project () =
  try
    (* dune-project is at the root of the interpreter package *)
    let rec find_dune_project dir depth =
      if depth > 10 then "dune-project" (* fallback *)
      else if Sys.file_exists (Filename.concat dir "dune-project") then
        Filename.concat dir "dune-project"
      else
        find_dune_project (Filename.concat dir "..") (depth + 1)
    in
    let dune_project_path = find_dune_project (Sys.getcwd ()) 0 in
    let ic = open_in dune_project_path in
    let content = ref "" in
      (* Read entire file *)
      (try
         while true do
