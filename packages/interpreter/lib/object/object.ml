...
type lobject =
  ...
  | Array of lobject array (** Fixed-size array of MLisp values *)
...
(** Convert OCaml array to MLisp list.
    (array_to_list [|1;2;3|]) -> (1 2 3) *)
let array_to_list arr =
  let len = Array.length arr in
  let rec aux i =
    if i >= len then Nil
    else Pair (arr.(i), aux (i + 1))
  in
    aux 0
;;
...
let rec string_object e =
  ...
  in
    ...
    | Array arr ->
      let len = Array.length arr in
      let elems = List.init len ~f:(fun i -> string_object arr.(i)) in
      let sep = ", " in
        [%string "#<array:%{Int.to_string len}[%{String.concat ~sep elems}]>"]
;;
