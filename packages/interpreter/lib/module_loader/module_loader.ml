...
let default_search_paths () =
  ...
  let home_dir = 
    ...
    | None -> current_dir
  in
  let home_packages = Filename.concat home_dir ".mlisp/packages" in
  let home_modules = Filename.concat home_packages "modules" in
  [ current_dir; modules_dir; home_packages; home_modules ]
;;
