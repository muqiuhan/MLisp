...
(* Batch mode: read a new input_stream from stdin if current one is exhausted *)
let get_batch_stream current_stream =
  if is_exhausted current_stream then (
    match In_channel.input_line In_channel.stdin with
    | Some line -> 
      let new_stream = Mlisp_utils.Stream_wrapper.make_stringstream line in
      new_stream
...
let rec repl stream env ~batch_mode ~has_error =
  (* Only set up LNoise in interactive REPL mode to avoid blocking in file/batch mode *)
  if stream.repl_mode && not batch_mode then (
  ...
  let input_stream =
    if stream.repl_mode && not batch_mode then (
      ...
      in
      let lines = if List.is_empty lines then [ input ] else lines in
        stream.recent_input <- lines;
        input_stream
    ) else if batch_mode && stream.repl_mode then (
      get_batch_stream stream
    ) else (
      stream
    )
  in
    try
      ...
      let _ = input_stream.column := saved_column in
        ...
        let result, env' = Eval.eval ast env in
          ...
          repl stream env' ~batch_mode ~has_error
    with
    | Stream.Failure ->
      if stream.repl_mode then
    ...
    | End_of_file ->
      if not batch_mode then print_endline "Goodbye!"
    | e ->
      raise e
;;
