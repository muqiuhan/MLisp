  ...
  let main_message, marker_message =
    ...
    | title :: "" :: rest ->
      ...
      let rec split_at_solutions acc = function
        ...
        | line :: rest when String.is_prefix line ~prefix:"Possible solutions:"
          ...
          (main, if String.is_empty explanation then message else explanation)
        | line :: rest ->
      ...
      if String.is_prefix message ~prefix:"Not found: " then (
        ...
        let is_in_comment line pos =
          let rec check_comment i =
            if i < 0 then false
            else if i > 0 && Char.(String.get line (i - 1) = ';') && Char.(String.get line i = ';') then true
            else if i >= 0 && Char.is_whitespace (String.get line i) then check_comment (i - 1)
            else false
          in
          check_comment (pos - 1)
        in
              ...
              let rec search_from pos =
                if pos + symbol_len > line_len then
                  ...
                  for i = 0 to symbol_len - 1 do
                    if pos + i >= line_len || Char.(String.get line (pos + i) <> String.get symbol_name i) then
                      matches := false
                  done;
                  if !matches then (
                    (* Found potential match - check word boundaries and that it's not in a comment *)
                    let before = if pos > 0 then String.get line (pos - 1) else ' ' in
                    let after_pos = pos + symbol_len in
                    let after = if after_pos < line_len then String.get line after_pos else ' ' in
                    let is_word_char c = Char.is_alphanum c || Char.equal c '_' in
                    if (not (is_in_comment line pos)) && (not (is_word_char before)) && (not (is_word_char after)) then
        ...
        let find_import_lines () =
          ...
          List.iteri source_lines ~f:(fun idx line ->
            if (not (is_comment_line line)) && String.is_substring line ~substring:"import" then
