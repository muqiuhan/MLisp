...
type cache_state =
  ...
  ; currently_loading : string list
    (** Modules currently being loaded (for circular detection) *)
  }

let global_cache : cache_state ref =
  ref { cache = Hashtbl.create ~size:100 (module String); currently_loading = [] }
;;

(** {2 Cache Management Functions} *)

(** Clear all entries from the module cache *)
let clear_cache () = Hashtbl.clear !global_cache.cache

(** Get the number of cached modules *)
let get_cache_stats () = Hashtbl.length !global_cache.cache

(** Check if a module is in the cache *)
let is_cached module_name = Hashtbl.mem !global_cache.cache module_name

(** Get a cached module entry *)
let get_cached module_name = Hashtbl.find !global_cache.cache module_name

(** Get the global cache state (for use by module_loader) *)
let get_global_cache () = global_cache
...
let register_cached_module
      module_name
      (module_obj : Object.lobject)
      (module_env : Object.lobject Object.env)
      (source_path : string)
  : unit
  =
  let cache_entry =
